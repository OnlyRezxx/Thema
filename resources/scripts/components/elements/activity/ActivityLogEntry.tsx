...
export default ({ activity, children, type }: Props) => {
    const { pathTo } = useLocationHash();
    const actor = activity.relationships.actor;
    const properties = wrapProperties(activity.properties);

    return (
        <div className={'flex justify-between items-center px-3 py-2 border-b-2 border-gray-800 last:border-0'+((type && type == 'profile') && ' px-0')}>
            <div>
